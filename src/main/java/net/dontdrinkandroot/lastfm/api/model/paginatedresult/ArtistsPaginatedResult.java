...
public class ArtistsPaginatedResult extends GenericPaginatedResult<List<Artist>> {

	public ArtistsPaginatedResult(Element element) {

		super(element);

		final ArrayList<Artist> artists = new ArrayList<Artist>();
		final List<Element> artistElements =
				net.dontdrinkandroot.utils.xml.DomUtils.getChildrenByTagName(element, "artist");
		for (final Element artistElement : artistElements) {
			artists.add(new Artist(artistElement));
		}
		this.setEntries(artists);
	}
